...
use super::*;
use crate::peer_manager::{PeerManagerNotification, PeerManagerRequest, PeerManagerRequestSender};
...
fn setup_discovery(
    ...
    let (network_reqs_tx, network_reqs_rx) = libra_channel::new(QueueStyle::FIFO, 1, None);
    let (conn_mgr_reqs_tx, conn_mgr_reqs_rx) = channel::new_test(1);
    let (network_notifs_tx, network_notifs_rx) = libra_channel::new(QueueStyle::FIFO, 1, None);
    ...
    let discovery = {
        Discovery::new(
            ...
            trusted_peers,
            ticker_rx,
            DiscoveryNetworkSender::new(PeerManagerRequestSender::new(network_reqs_tx)),
