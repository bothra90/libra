    ...
    TMuxer: StreamMultiplexer + 'static,
{
    ...
    async fn handle_connection_event(&mut self, event: ConnectionNotification<TMuxer>) {
    ...
    async fn add_peer(
        ...
        let (network_reqs_tx, network_notifs_rx) = NetworkProvider::new(
            self.executor.clone(),
            identity.clone(),
            address.clone(),
            origin,
    ...
    fn spawn_peer_network_events_handler(
        ...
        self.executor.spawn(
            network_events
                .map(move |inbound_event| {
                    Ok(Self::handle_inbound_event(
                        inbound_event,
                        peer_id,
