// TODO(abhayb): Add comment.

// TODO(abhayb): use networkerror or peererror instead of PeerManagerError.
...
// TODO(abhayb): Remove PeerId from enum variants since they're no longer needed.
#[derive(Debug)]
    ...
    TMuxer: StreamMultiplexer,
{
    ...
    /// Flag to indicate if the connection was explicitly closed.
    shutting_down: bool,
}
                ...
                match inbound_substream {
                  Ok(negotiated_substream) => {
                    ...
                    if self.rpc_protocols.contains(&protocol) {
                      ...
                      self.direct_send_protocols.contains(&protocol) {
                        if let Err(err) = self.direct_send_notifs_tx.send(event).await {
                            warn!("Failed to send notification to DirectSend actor. Error: {:?}", err);
                        }
                      } else {
