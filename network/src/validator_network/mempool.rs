...
/// turn a thin wrapper around a `channel::Sender<PeerManagerRequest>`, so it is
...
mod tests {
    ...
    fn test_mempool_network_sender() {
        ...
        let mut sender = MempoolNetworkSender::new(PeerManagerRequestSender::new(network_reqs_tx));
