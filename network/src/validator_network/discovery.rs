...
///
/// `DiscoveryNetworkEvents` is a `Stream` of `MultiNetworkNotification` where the
/// raw `Bytes` rpc messages are deserialized into
/// `DiscoveryMsg` types. `DiscoveryNetworkEvents` is a thin wrapper
/// around a `channel::Receiver<MultiNetworkNotification>`.
...
/// in turn a thin wrapper around a `channel::Sender<PeerManagerRequest>`, so it is
